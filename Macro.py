...
my_list = parse_table(tabla)
sht1 = client.open("Info Macro")
worksheet4 = sht1.get_worksheet(4)

delete = worksheet4.range('B3:I500')
for cell in delete:
    cell.value = ""
    worksheet4.update_cells(delete)

def table_sheets_01(list):
    for row in range(len(list)):
        worksheet4.update_cell(row+2,2,my_list[row][0])
        worksheet4.update_cell(row+2,3,my_list[row][1])

def table_sheets_02(list):
    for row in range(len(list)):       
        worksheet4.update_cell(row+2,4,my_list[row][2])
        worksheet4.update_cell(row+2,5,my_list[row][3])

def table_sheets_03(list):
    for row in range(len(list)):         
        worksheet4.update_cell(row+2,6,my_list[row][4])
        worksheet4.update_cell(row+2,7,my_list[row][5])
        
def table_sheets_04(list):
    for row in range(len(list)):         
        worksheet4.update_cell(row+2,8,my_list[row][6])
        worksheet4.update_cell(row+2,9,my_list[row][7])        
        
        cpd_30 = worksheet4.cell(2, 13).value
        cpd_60 = worksheet4.cell(2, 14).value
        cpd_180 = worksheet4.cell(2, 15).value
        cpd_360 = worksheet4.cell(2, 16).value
        
        sht5 = client.open("Info Macro")
        worksheet5 = sht5.get_worksheet(5)
        worksheet5.update_cell(3, 13, cpd_30)
        worksheet5.update_cell(3, 14, cpd_60)
        worksheet5.update_cell(3, 15, cpd_180)
        worksheet5.update_cell(3, 16, cpd_360)        
        
table_sheets_01(my_list)
table_sheets_02(my_list)
table_sheets_03(my_list)
table_sheets_04(my_list)
...
my_list_final = [w.replace('%', '') for w in my_list_final]
num_cpd = np.array(my_list_final[8:])
cpd_reshaped = num_cpd.reshape(113,8)
cpd_final = pd.DataFrame(cpd_reshaped, columns=['Plazo', 'Fecha', 'Tasa_Min', 'Tasa_Max', 'Tasa_Prom', 
                                                          'Monto_Nom', 'Monto_Liq', 'Cheques'])
...
print (tasa_final_30)
print(tasa_final_60)
print(tasa_final_180)
print(tasa_final_360)
print(tasa_final_mayor_360)
print (volumen_total)
print (cheques_totales)
...
emitidos_final[["Neto_gravado","Neto_no_gravado","Exento",
        "IVA", "Total"]] = emitidos_final[["Neto_gravado","Neto_no_gravado",
                                           "Exento","IVA", "Total"]].apply(pd.to_numeric)
